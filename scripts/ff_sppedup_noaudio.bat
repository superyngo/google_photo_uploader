...
if "%~1"=="" (
    ...
    for %%F in (*.mkv) do (
        echo Processing: %%F
        ffmpeg -i "%%F" -filter_complex "[0:v]setpts=!video_pts!*PTS[v];[0:a]atempo=!speedup_ratio![a]" -map "[v]" -map "[a]" "%%~nF_speedup.mkv"
    )
) else (
    ...
    if "%~1"=="" goto :done
    echo Processing: %~1
    ffmpeg -i "%~1" -filter_complex "[0:v]setpts=!video_pts!*PTS[v];[0:a]atempo=!speedup_ratio![a]" -map "[v]" -map "[a]" "%~n1_speedup.mkv"
    shift
    goto :process_args
)
:done
