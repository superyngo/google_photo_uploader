...
def list_video_files(
    root_path: str, valid_extensions: set[str] | None = None
) -> list[str]:
...
def merge_videos(
    ...
    base_path: str,
    handle_speedup: HandleSpeedup | None = None,
) -> int:
    ...
    for date_str, videos in video_dict.items():
        ...
        try:
            ...
            logger.error(
                f"Failed to concatenate videos for {date_str}. Error: {e.stderr.decode()}"
            )
            return 1
        ...
        try:
            ...
            logger.error(
                f"Failed to delete files or directories for {date_str}: {str(e)}"
            )
            return 2

        logger.info(
            f"Processed {date_str}, saved to {output_file}, set timestamps, and deleted original files."
        )
        ...
        if handle_speedup is not None:
            ...
            speedup_output_file: str = os.path.join(
                speedup_output_dir, f"{base_name}_speedup{extension}"
            )
            encode_info: ffmpeg_converter.EncodeKwargs = {
                "video_track_timescale": 90000,
                "vcodec": "hevc",
                "video_bitrate": 920811,
                "acodec": "pcm_alaw",
                "ar": 16000,
                "f": "mov",
            }
            do_speedup: int = ffmpeg_converter.speedup(
                output_file, speedup_output_file, 100, **encode_info
            )

            # Set the file's timestamp to the first video's epoch time
            os.utime(speedup_output_file, (first_video_epoch, first_video_epoch))
            logger.info(
                f"Sped up {output_file} to {speedup_output_file} with same timestamps"
            )

    return 0


def handler(folder_path:Path){
